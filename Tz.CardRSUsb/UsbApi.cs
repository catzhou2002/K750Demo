...
namespace Tz.CardRS
{
    internal class UsbApi
    {
        ...
        public static List<string> GetUsbFileNames(string vid = null, string pid = null)
        {
            ...
            {
                string deviceVIDPID = string.IsNullOrEmpty(vid) ? null : ("vid_" + vid + "&pid_" + pid);
                ...
                while (SetupDiEnumDeviceInterfaces(hidInfoSet, IntPtr.Zero, ref hidGuid, index, ref interfaceInfo))
                {
                    ...
                    if (SetupDiGetDeviceInterfaceDetail(hidInfoSet, ref interfaceInfo, detailDataBuffer, bufferSize, ref bufferSize, null))// strtInterfaceData))
                    {
                        ...
                        if (deviceVIDPID == null || devicePath.IndexOf(deviceVIDPID) >= 0)
                        {
        ...
        /// 
        /// </summary>
        /// <param name="vid">Vendor ID，供应商识别码</param>
        /// <param name="pid">Product ID，产品识别码</param>
        public UsbApi(string vid, string pid) : this(GetUsbFileNames(vid, pid).FirstOrDefault())
        { }
        /// <summary>
        /// 
        /// </summary>
        ...
        /// </summary>
        /// <param name="array"></param>
        public byte[] WriteThenRead(byte[] data)
        {
            Write(data);
            byte[] inBuffer = new byte[_InputBufferSize];
            var task = _UsbFileStream.ReadAsync(inBuffer, 0, _InputBufferSize);
            task.Wait();
            return inBuffer;
        }
